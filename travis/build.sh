...
case "${TRAVIS_OS_NAME}" in
    ...
    linux)
        ...
        docker run -it --name builder -e CC_VER=${CC_VER} -e CMAKE_ARGS=${CMAKE_ARGS} -e TEST=${TEST} -e DEPLOY=${DEPLOY} -e BUILD=${BUILD} -e GPGKEY=84B8A307 ${SHARE_SRC} ${SHARE_SECRETS} builder-${PLATFORM} bash
