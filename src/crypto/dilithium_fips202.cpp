              ...
              unsigned long long input_length)
{
    ...
    const auto blocks = output_length / SHAKE128_RATE;
    shake128_absorb(state, input, input_length);
    shake128_squeezeblocks(output, blocks, state);

    const auto produced = blocks * SHAKE128_RATE;
    output_length -= produced;
    if (output_length != 0) {
        output += produced;
        shake128_squeezeblocks(block, 1, state);
              ...
              unsigned long long input_length)
{
    ...
    shake256_absorb(state, input, input_length);
    shake256_squeezeblocks(output, blocks, state);

    const auto produced = blocks * SHAKE256_RATE;
    output_length -= produced;
    if (output_length != 0) {
        output += produced;
        shake256_squeezeblocks(block, 1, state);
        std::memcpy(output, block, static_cast<std::size_t>(output_length));
    }
}
