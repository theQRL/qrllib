...
func TestDilithiumReference(t *testing.T) {
	d := dilithium.NewDilithium__SWIG_0()

	if d.GetPK().Size() != 1472 {
		t.Errorf("Incorrect PK Size\nExpected: %d\nFound: %d", 1472, d.GetPK().Size())
	}

	if d.GetSK().Size() != 3504 {
		t.Errorf("Incorrect SK Size\nExpected: %d\nFound: %d", 3504, d.GetSK().Size())
	}
	...
	if messageOut.Size() != int64(len(message)) {
		t.Errorf("Incorrect Message Size\nExpected: %d\nFound: %d", int64(len(message)), messageOut.Size())
	}

	if signatureOut.Size() != 2701 {
		t.Errorf("Incorrect signatureOut Size\nExpected: %d\nFound: %d", 2701, signatureOut.Size())
	}

	if !reflect.DeepEqual(message, misc.UCharVectorToBytes(messageOut)) {
		t.Errorf("Message doesn't match\nExpected: %s\nFound: %s", message, misc.UCharVectorToBytes(messageOut))
	}
}

func TestDilithiumReference2(t *testing.T) {
	pk := goqrllib.Hstr2bin(PK1_HSTR)
	sk := goqrllib.Hstr2bin(SK1_HSTR)

	d := dilithium.NewDilithium__SWIG_1(pk, sk)
